    ...
    stages {
        ...
        stage('Run Tests') {
            steps {
                ...
                // ✅ EXTENT REPORT HANDLING (FULL ASSETS)
                script {
                    def reportDate = new Date().format('yyyy-MM-dd')
                    def reportBaseDir = "reports/extentReports/${reportDate}"

                    // The folder generated by Extent must include index.html + css + js + images
                    def suiteFolder = ""
                    ...
                    echo "Publishing full Extent report from: ${fullReportDir}"
